...
use p3_field_testing::{benchmark_add, benchmark_mul_latency, benchmark_mul_throughput};
...
fn bench_quartic_extension(c: &mut Criterion) {
    ...
    benchmark_mul_latency::<EF4>(c, name);
    benchmark_mul_throughput::<EF4>(c, name);
}
...
fn bench_qunitic_extension(c: &mut Criterion) {
    ...
    benchmark_mul_latency::<EF5>(c, name);
    benchmark_mul_throughput::<EF5>(c, name);
}
